...
INSERT INTO partners (
  ...
  (1, 1, '佐藤 健', 'さとう けん', 30, '1995-04-15', 1, '2023-01-10', '2023-07-10', '爽やか', 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 1, 2, 1, 1, 0, 2, 3, '2025-07-20 10:00:00', '2025-07-20 10:00:00'),
  (2, 1, '鈴木 玲子', 'すずき れいこ', 28, '1997-02-20', 3, '2024-02-15', '2025-01-15', '優しい', 2, 2, 3, 3, 2, 3, 3, 2, 3, 2, 0, 1, 0, 3, 1, 2, 2, '2025-07-19 09:30:00', '2025-07-19 09:30:00'),
  (3, 2, '高橋 真', 'たかはし まこと', 35, '1990-11-05', 2, '2023-05-01', '2023-10-01', '頼もしい', 3, 3, 2, 3, 3, 3, 3, 3, 3, 3, 1, 3, 0, 1, 3, 0, 1, '2025-07-18 08:20:00', '2025-07-18 08:20:00'),
  (4, 3, '田中 美咲', 'たなか みさき', 26, '1998-07-22', 4, '2024-06-10', '2025-06-10', '明るい', 2, 3, 3, 2, 3, 2, 3, 3, 2, 3, 0, 1, 1, 0, 2, 3, 3, '2025-07-17 12:00:00', '2025-07-17 12:00:00'),
  (5, 2, '井上 翔太', 'いのうえ しょうた', 32, '1992-03-30', 5, '2023-03-25', '2025-03-25', '誠実', 3, 2, 3, 3, 3, 3, 2, 3, 3, 2, 0, 0, 1, 2, 1, 2, 2, '2025-07-16 14:45:00', '2025-07-16 14:45:00');
...
/*phase_status の値は、0 = 未到達、1 = 進行中、2 = 完了で設定しています。*/
  ...
  (5, 4, 0, 0, '相手の趣味について話す', 15, '2025-07-12 12:00:00', '2025-07-12 12:00:00');
  /* check は 0（未終了）、1（終了）を使っています。

type は 0（タスク）、1（イベント）です。

phase_id は恋愛フェーズのIDに対応させています。 */
...
/* question_id は質問はQ1, Q2、結論はA1〜A3としました。

yes_next_id と no_next_id で次の質問や結論に分岐しています。

prev_id で前の質問IDをリンクしています。

conclusion は結論の場合に記述、それ以外はNULLです。

登録・更新日時は適当に2025年7月の日時を設定しました */
  
  
  
  
  INSERT INTO marriage_plans (
  id, users_id, marriage_timing, span_id, span, percentage, created_at, updated_at
) VALUES
  (1, 101, '2025-09-01', 1, 3, 0.25, '2025-07-01 09:00:00', '2025-07-01 09:00:00'),
  (2, 102, '2025-10-01', 2, 6, 0.30, '2025-07-02 10:00:00', '2025-07-02 10:00:00'),
  (3, 103, '2025-11-01', 3, 12, 0.20, '2025-07-03 11:00:00', '2025-07-03 11:00:00'),
  (4, 104, '2026-01-01', 4, 24, 0.15, '2025-07-04 12:00:00', '2025-07-04 12:00:00'),
  (5, 105, '2026-03-01', 5, 36, 0.10, '2025-07-05 13:00:00', '2025-07-05 13:00:00');
/* users_id はユーザーごとのID（例として101〜105）を設定。

marriage_timing は「提案用婚活希望時期」として日付（yyyy-mm-dd形式）を設定。

span_id は期間分類のID（例: 1〜5）。

span は期間（月数などの単位想定）で設定。

percentage は割合で0〜1の範囲で設定。

登録・更新日時は2025年7月の任意日時です。 */
  
 
  
  INSERT INTO messages (
  id, users_id, partners_id, mood, matter, prompt, created_at, updated_at
) VALUES
  (1, 101, 201, 1, 1, '初めまして！趣味についてお話しませんか？', '2025-07-20 10:00:00', '2025-07-20 10:00:00'),
  (2, 102, 202, 2, 2, 'こんにちは。最近観た映画について語り合いましょう。', '2025-07-21 11:00:00', '2025-07-21 11:00:00'),
  (3, 103, 203, 3, 1, 'お疲れ様です！週末の予定を教えてください。', '2025-07-22 12:00:00', '2025-07-22 12:00:00'),
  (4, 104, 204, 2, 3, 'はじめまして。旅行の話をしましょうか？', '2025-07-23 13:00:00', '2025-07-23 13:00:00'),
  (5, 105, 205, 1, 2, '最近読んだ本について語り合いませんか？', '2025-07-24 14:00:00', '2025-07-24 14:00:00');
/* users_id と partners_id はそれぞれユーザーIDとお相手のIDを仮に設定しています。

mood と matter は用途に応じて数値を入れています（例: 雰囲気の種類や用件の種類など）。

prompt はメッセージの提案文です。

登録日時・更新日時は2025年7月の任意の日時にしています。 */